...
func (c DropJobTempAccount) DropUser() error {
	drsRes, err := drs.RPCMySQL(
		c.BkCloudId,
		c.Addresses,
		[]string{
			fmt.Sprintf(
				`CALL infodba_schema.dba_drop_job_temp_account('%s')`,
				c.User,
			),
		},
		true,
	...
	if len(reports) > 0 {
		slog.Info("drop job temp account", slog.Any("reports", reports))
	...
	slog.Info("drop job temp account finish")
	return nil
}
