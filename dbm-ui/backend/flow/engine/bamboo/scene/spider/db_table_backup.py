...
class TenDBClusterDBTableBackupFlow(object):
    ...
    def backup_flow(self):
        ...
        for cluster_id, jobs in merged_jobs.items():
            cluster_flow.append(
                self._build_cluster_sub_flow(cluster_id=cluster_id, jobs=jobs).build_sub_process(sub_name=_(""))
            )
    ...
    def _build_cluster_sub_flow(self, cluster_id: int, jobs: List):
        ...
        for job in jobs:
            ...
            cluster_flow.add_sub_pipeline(sub_flow=job_flow.build_sub_process(sub_name=_("")))
        return cluster_flow
